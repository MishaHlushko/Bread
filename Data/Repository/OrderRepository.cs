...
namespace Beads_Store.Data.Repository
{
    public class OrderRepository : IOrderRepository
    {
        ...
        {
            return await _appDBContext.orders.Include(s=>s.StatusOrder).Include(o=>o.orderDetails).ThenInclude(m=>m.beads).ToListAsync();
        }
        public async Task<List<OrderDetail>> GetOrdersDetailsAsync()
        {
            return await _appDBContext.orderDetails.Include(m => m.beads)
                .Include(o => o.order)
        ...
        public async Task CreateOrderAsync(Order order, string userId)
        {
            ...
            foreach (var el in items)
            {
                var orderDetail = new OrderDetail()
                {
                    ...
                    beads = el.beads,
                    order = order,
                    price = el.beads.price,
                    
                };
                _appDBContext.Beads.FirstOrDefault(p => p.Id == orderDetail.beads.Id).quantity--;
        ...
        {
            List<Order> orderDetails = await _appDBContext.orders.Include(s => s.StatusOrder).Include(o => o.orderDetails).ThenInclude(m => m.beads).Where(o => o.userId == userId).ToListAsync();
        ...
        {
            IQueryable<Order> orderDetails = _appDBContext.orders.Include(s => s.StatusOrder).Include(o => o.orderDetails).ThenInclude(m => m.beads).Where(o =>o.statusOrderId == statusOrderId);
        ...
        {
            IQueryable<Order> orderDetails = _appDBContext.orders.Include(s => s.StatusOrder).Include(o => o.orderDetails).ThenInclude(m => m.beads).Where(o => o.statusOrderId == statusOrderId).Where(o => o.userId == userId);
